...
{
inline constexpr std::size_t BackgroundDitherSize = 8;
...
[[nodiscard]] constexpr std::uint8_t GetBackgroundDitherRank(const std::size_t X, const std::size_t Y) noexcept
{
	return BackgroundDitherPattern[(Y % BackgroundDitherSize) * BackgroundDitherSize + X % BackgroundDitherSize];
}

[[nodiscard]] constexpr float QuantizeBackgroundChannel(const float Value, const std::size_t X, const std::size_t Y) noexcept
{
